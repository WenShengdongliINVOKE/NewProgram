namespace 小玩意
{
    ...
    internal class Message
    {
        ...
        /// <returns></returns>
        public static byte[] MessageGeneration(int slaveStation, byte mode, short startAdr, object value)
        {
            //C# 8.0以下版本的写法：
            switch (mode)
            {
        ...
        /// <returns></returns>
        public static byte[] GetReadMessage(int slaveStation, byte mode, short startAdr, short length)
        {
            //定义临时字节列表
            List<byte> temp = new List<byte>();
